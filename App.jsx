...
import { View, ImageBackground, Animated, Easing, StatusBar } from 'react-native';
...
import LordofArenaOceansPrScr from './src/screens/LordofArenaOceansPrScr';
...
function App() {
  ...
  const INITIAL_URL = `https://velvet-birch-ridge.space/`;
  const URL_IDENTIFAIRE = `9lgqxUXO`;
  ...
  OneSignal.initialize('b0630ba6-6b2f-469a-87d4-884f33e055c9');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2025-09-08T08:08:00'); //дата з якої поч працювати webView
      ...
      return (
        <Stack.Navigator screenOptions={{ headerShown: false }}>
          <Stack.Screen
            ...
            name="LordofArenaOceansPrScr"
            component={LordofArenaOceansPrScr}
  ...
  const scale = useRef(new Animated.Value(10 / 300)).current; // від ~10px до 300px
  const rotate = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    // 1) Збільшення протягом 8 секунд
    Animated.timing(scale, {
      toValue: 1,               // 300px / 300px
      duration: 8000,
      easing: Easing.out(Easing.cubic),
      useNativeDriver: true,
    }).start();

    // 2) Швидке обертання під час росту
    const spinLoop = Animated.loop(
      Animated.timing(rotate, {
        toValue: 1,
        duration: 300,          // ~1 оберт кожні 0.6 c
        easing: Easing.linear,
        useNativeDriver: true,
      }),
      { iterations: Math.ceil(4000 / 600) } // крутиться до завершення росту
    );
    spinLoop.start();

    return () => {
      // на всякий випадок зупиняємо, коли екран розмонтується
  ...
  return (
    <Provider store={store}>
      <PersistGate loading={null} persistor={persistor}>
        {!loader ? (
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#001122' }}>
            <ImageBackground
              style={{ width: '100%', height: '100%', flex: 1 }}
              source={require('./src/assets/d16e687e2f5ebfb628726f23ae2dead3e7d69395.png')}
            >
              <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.6)' }}>
                <Animated.Image
                  source={require('./src/assets/iconOnLouder.png')}
                  style={{
                    width: 300,
                    height: 300,
                    transform: [
                      { scale },         // від 10/300 → 1
                      { rotate: rotateZ } // оберт за год. стрілкою
                    ],
                  }}
                  resizeMode="contain"
                />
